...
from db.db import db_get_figi, get_all_tickers, get_sma, get_t_token
from bot.bot import bot
from telebot import types
from graphics.sma_graph import plot_sma
...
def mls_sma_handler(call):
    ...
    if token is not None:
        ...
        if not tickers:
            ...
            buttons = [
                types.InlineKeyboardButton(text='пол года', callback_data='sma_interval_6'),
                types.InlineKeyboardButton(text='год', callback_data='sma_interval_12')
            ]
    ...
    if token is not None:
        ...
        if not tickers:
            ...
            for ticker in tickers:
                button = types.InlineKeyboardButton(text=str(ticker[0]), callback_data=f'mls_sma_ticker_{ticker[0]}')
