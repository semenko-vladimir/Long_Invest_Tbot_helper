...
from app.client.graphics.macd_graph import plot_macd
...
def mls_macd_handler(call):
    """
    Обработчик для расчета MACD сигнала.
    ...
    try:
        ...
        if not instruments:
            ...
            buttons = [
                types.InlineKeyboardButton(text='📅 6 месяцев', callback_data='macd_interval_6'),
                types.InlineKeyboardButton(text='📆 1 год', callback_data='macd_interval_12')
            ]
    ...
    try:
        ...
        if not instruments:
            ...
            for instrument in instruments:
                ...
                button = types.InlineKeyboardButton(text=ticker, callback_data=f'mls_macd_ticker_{ticker}')
                inline_keyboard.add(button)
...
def calculate_mls_macd(call):
    """
    Обработчик для расчета MACD сигнала для выбранного инструмента.
    
    Получает данные о свечах, рассчитывает MACD сигнал и отображает график.
    """
    chat_id = call.message.chat.id
    ticker = call.data.replace('mls_macd_ticker_', '')
    
    try:
        send_or_edit_message(chat_id, f"⏳ *Обработка запроса*\n\nРассчитываем MACD для тикера `{ticker}`...")
        ...
        # Получаем настройки MACD
        macd_settings = signals_client.get_signal_macd()
        
        if not macd_settings:
            send_or_edit_message(chat_id, "❌ *Ошибка*\n\nНастройки MACD не найдены. Пожалуйста, настройте сигнал MACD.")
            return
        
        macd_fast = macd_settings.get('fastLength')
        macd_slow = macd_settings.get('slowLength')
        macd_signal_length = macd_settings.get('signalLength')
        ...
        min_candles = max(macd_fast, macd_slow, macd_signal_length) + 1
        
        if len(df["high"].values) < min_candles:
            send_or_edit_message(chat_id, "❌ *Ошибка*\n\nНедостаточно свечей для расчета сигнала MACD.")
            return
        ...
        if macd_signal != 'hold':
            emoji = "🟢 BUY" if macd_signal == 'buy' else "🔴 SELL"
            send_or_edit_message(chat_id, f'📊 *MACD сигнал для {ticker}*: {emoji}')
        
        # Строим график MACD
        plot_macd(chat_id, df)
    
    except Exception as e:
        send_or_edit_message(chat_id, f"❌ *Ошибка при расчете MACD сигнала*\n\n`{str(e)}`")
