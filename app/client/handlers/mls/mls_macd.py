...
def calculate_mls_macd(call):
    ...
    try:
        ...
        if not macd_settings:
            bot.send_message(chat_id, "Настройки MACD не найдены. Пожалуйста, настройте сигнал MACD.")
            return
        ...
        if len(df["high"].values) < min_candles:
            bot.send_message(chat_id, "Недостаточно свечей для расчета сигнала MACD.")
            return
        ...
        bot.send_message(chat_id, f"Ошибка при расчете MACD сигнала: {str(e)}")
